...
const CurvedLoop = ({
  ...
  speed = 2,
  className,
  textColor = "#1e293b", // Default to slate-800
  curveAmount = 200, // Reduced curve for a more subtle effect
  direction = "left",
  interactive = true,
}) => {
  const text = useMemo(() => {
  ...
  useEffect(() => {
    ...
    const step = () => {
      tspansRef.current.forEach((t) => {
        ...
        if (!dragRef.current) x += delta;

        const totalWidth = tspansRef.current.length * spacing;
        x = ((x + totalWidth) % totalWidth) - spacing; // Proper looping logic
  ...
  return (
    ...
    >
      <svg
        className="select-none w-full overflow-visible block aspect-[100/12] text-[clamp(2rem,10vw,5rem)] font-bold tracking-[3px] uppercase leading-none"
